...
const skills = await Promise.all(catalog.skills.map(async (skill) => ({ ...skill, files: await Promise.all([...skill.files].sort((left, right) => left.path.localeCompare(right.path)).map(async (file) => ({
